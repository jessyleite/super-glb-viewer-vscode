import { W as c } from "./WebViewAdapter-BgV_RHrL.js";
import { g as y, r as f, a as m } from "./index--tWw_-Xm.js";
class b extends c {
  unityInstance = null;
  canvas = null;
  getConfig() {
  ...
  async init(t, i) {
    ...
    const n = new Promise((a, l) => {
      const u = setTimeout(
        () => l(new Error(`Unity did not become ready within ${e.readyTimeout}ms`)),
        e.readyTimeout
      );
  ...
  // ── Helpers ─────────────────────────────────────────────────────
  resolveBuildDir(t) {
    const i = window.__PUBLIC_BASE_URI__, e = i ? f(t.buildUrl, i) : m(t.buildUrl);
