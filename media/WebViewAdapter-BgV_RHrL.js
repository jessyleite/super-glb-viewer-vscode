import { eo as p, r as f, a as u, ep as y, eq as m, er as w } from "./index--tWw_-Xm.js";
const c = "super-glb-viewer";
function g(l) {
  return l.data?.source === c;
}
class E {
  iframe = null;
  container = null;
  ...
  async init(e, t) {
    ...
    this.iframe = document.createElement("iframe"), this.iframe.className = s.iframeClass, this.iframe.style.cssText = "width:100%;height:100%;border:none;display:block;";
    const a = typeof window < "u" && window.__PUBLIC_BASE_URI__;
    if (a) {
      const i = f(s.buildUrl, a), d = i.substring(0, i.lastIndexOf("/") + 1), r = (await fetch(i).then((o) => o.text())).replace("<head>", '<head><base href="' + d + '">');
      this.iframe.src = URL.createObjectURL(new Blob([r], { type: "text/html" }));
    } else
      this.iframe.src = u(s.buildUrl);
    this.container.appendChild(this.iframe), await new Promise((i, d) => {
      const h = setTimeout(
        () => d(new Error(`WebView engine did not become ready within ${s.readyTimeout}ms`)),
        s.readyTimeout
      );
  ...
  handleGuestEvent(e) {
    switch (e.type) {
      ...
      case "morphTargetUpdate": {
        const t = w();
        if (t) {
          const s = new Map(Object.entries(e.state));
          t(s);
        }
        break;
      }
      ...
      case "error":
        ...
        for (const [t, s] of this.waiters)
          s.reject(new Error(e.message)), this.waiters.delete(t);
        break;
    }
  }
  // ── PostMessage helpers ──────────────────────────────────────────
  ...
  resolveWaiter(e, t) {
    const s = this.waiters.get(e);
    s && (s.resolve(t), this.waiters.delete(e));
  }
  waitForEvent(e, t = 6e4) {
    return new Promise((s, a) => {
      const n = setTimeout(() => {
        this.waiters.delete(e), a(new Error(`WebViewAdapter: timeout waiting for "${e}"`));
      }, t);
      this.waiters.set(e, {
        resolve: (i) => {
          clearTimeout(n), s(i);
  ...
  async loadModel(e, t) {
    if (this.isSameOrigin())
      ...
      this.send({ type: "loadModelBuffer", buffer: a }, [a]);
    }
  ...
  async loadEnvironment(e) {
    if (this.isSameOrigin())
      ...
      this.send({ type: "loadEnvironmentBuffer", buffer: s }, [s]);
    }
  ...
  setCameraPosition(e, t, s) {
    this.send({ type: "setCameraPosition", position: e, target: t, instant: s });
  }
  setExposure(e) {
